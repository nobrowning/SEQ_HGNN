...
from data import ACMDataset, DBLPDataset, OAGFieldDataset, OAGVenueDataset
# from data import OAGVenueDataset
...
def load_pretrained_node_embed(g, node_embed_path, concat=False, use_raw_id=False):
    ...
    """
    if node_embed_path and '.model' not in node_embed_path:
        return load_pretrained_node_embed_from_tensor(g, node_embed_path, concat)
...
def load_pretrained_node_embed_from_tensor(g, node_embed_path, concat=False):
    """为没有输入特征的顶点加载预训练的顶点特征
    ...
    """
    for ntype in g.ntypes:
        embed = torch.load(os.path.join(node_embed_path, '{}.pt'.format(ntype))).float().to(g.device)
        if 'feat' in g.nodes[ntype].data:
            if concat:
                g.nodes[ntype].data['feat'] = torch.cat([g.nodes[ntype].data['feat'], embed], dim=1)
        else:
