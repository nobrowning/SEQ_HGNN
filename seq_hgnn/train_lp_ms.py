...
def train():
    ...
    for batch_idx in trange(args.n_batch, dynamic_ncols=True):
        ...
        batch_y = batch[targe_node_type][label_name_extend].long()
        
        optimizer.zero_grad()
        if scalar is not None:
            with torch.cuda.amp.autocast():
                logist = model(batch.x_dict, batch.edge_index_dict, batch_y, batch_train_mask, batch_size)
                loss = F.cross_entropy(logist[:batch_size], batch_y[:batch_size])
...
def test(loader):
    ...
    for batch in tqdm(loader, dynamic_ncols=True):
        ...
        batch_train_mask = batch[targe_node_type]['train_mask']
        batch_y_pseudo = batch[targe_node_type][label_name_extend].long()
        
        
        out = model(batch.x_dict, batch.edge_index_dict, batch_y_pseudo, batch_train_mask, batch_size)[:batch_size]
        pred = out.argmax(dim=-1)
        
        predict_confident = out.softmax(dim=1)
        predict_confident = predict_confident.max(1)[0]
        
        pred_list.append(pred)
        confident_list.append(predict_confident)
        true_list.append(batch[targe_node_type][label_name][:batch_size])
        node_id_list.append(batch[targe_node_type]['ids'][:batch_size])
    pred_list = torch.cat(pred_list, dim=-1)
    true_list = torch.cat(true_list, dim=-1)
    node_id_list = torch.cat(node_id_list, dim=-1)
    confident_list = torch.cat(confident_list, dim=-1)
    acc = accuracy(pred_list, true_list).item()
    f1 = f1_score(pred_list, true_list, average='macro', num_classes=num_classes)
    
    return acc, f1, node_id_list, pred_list, confident_list
...
def reset_train_dataloader(val_ids, test_ids, val_preds, test_preds, \
    val_confidents, test_confidents, threshold=0.75):
    train_input_nodes = torch.clone(data[targe_node_type].train_mask)
    data[targe_node_type][label_name_extend] = torch.clone(data[targe_node_type][label_name])
    val_confident_mask = val_confidents >= threshold
    test_confident_mask = test_confidents >= threshold
    selected_val_node_ids = val_ids[val_confident_mask]
    selected_test_node_ids = test_ids[test_confident_mask]
    train_input_nodes[selected_val_node_ids] = True
    train_input_nodes[selected_test_node_ids] = True
    data[targe_node_type][label_name_extend][selected_val_node_ids] = val_preds[val_confident_mask]
    data[targe_node_type][label_name_extend][selected_test_node_ids] = test_preds[test_confident_mask]
    train_loader = HGTLoader(data, num_samples=[args.num_samples] * (args.num_layers+1), \
            shuffle=True, input_nodes=(targe_node_type, train_input_nodes), batch_size=args.batch_size, \
            num_workers=args.workers)
    return train_loader

def build_optimizer(args, lr, model, num_batch, num_epochs):
    ...
    if args.n_batch is None or args.n_batch > num_batch:
        args.n_batch = num_batch
    optimizer = optim.AdamW(optimizer_grouped_parameters, lr=lr, eps=1e-06)
    total_steps = args.n_batch * num_epochs
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='train SeqHGNN+LP+MS')
    ...
    parser.add_argument('--logsubfix', type=str, default='lp_ms', help='log file subfix')
    parser.add_argument('--clip', type=float, default=1.0, help='Gradient Norm Clipping')
    parser.add_argument('--ct', type=float, default=0.75, help='confident_threshold')
    parser.add_argument('--stages', nargs='+', help='stages', type=int, required=False)
    ...
    parser.add_argument('--save-path', help='model save path', default='model/seq_hgnn_')
    parser.add_argument('--load-path', help='model load path')
    parser.add_argument('--off-wandb', default=False, action='store_true', help='if use wandb')
    args = parser.parse_args()
    tags = [t for t in args.logsubfix.split(',')]
    run = wandb.init(project='SEQ_HGNN', entity='nobrowning', tags=tags, mode="disabled" if args.off_wandb else None)
    ...
    # add pseudo labels
    label_name_extend = label_name+ '_'
    data[targe_node_type][label_name_extend] = torch.clone(data[targe_node_type][label_name])
    ...
    optimizer, lr_scheduler =  build_optimizer(args, args.lr, model, len(train_loader), args.stages[0])
    
    warnings.filterwarnings('ignore')
    best_val_acc, best_test_acc = 0, 0
    best_val_f1, best_test_f1 = 0, 0
    best_s_val_acc, best_s_test_acc = 0, 0
    best_epoch, total_epoch = 0, 0
    step = 0
    best_preds, best_confidents, eval_ids = None, None, None
    
    if args.load_path is not None:
        model.load_state_dict(torch.load(args.load_path, map_location=device))
        val_acc, val_f1, val_ids, val_preds, val_confidents = test(val_loader)
        test_acc, test_f1, test_ids, test_preds, test_confidents = test(test_loader)
        train_loader = reset_train_dataloader(val_ids, test_ids, val_preds, test_preds, val_confidents, test_confidents, threshold=args.ct)
        best_val_acc = val_acc
        best_test_acc = test_acc
        best_val_f1 = val_f1
        best_test_f1 = test_f1
    
    for stage_idx, stage_epochs in enumerate(args.stages):
        
        if stage_idx == 1:
            optimizer, lr_scheduler =  build_optimizer(args, 0.0002, model, len(train_loader), sum(args.stages[1:]))
        
        for epoch in range(stage_epochs):
            total_epoch += 1
            loss, train_acc, s_val_acc, s_test_acc, current_lr = train()
            wandb.log({"Loss/train": loss, 'LR': current_lr, 'Acc/train': train_acc}, step=total_epoch)
            logging.info(f'LR: {current_lr:.4f}, Epoch: {total_epoch:03d}, Loss: {loss:.4f}, Train: {train_acc:.4f}, sVal: {s_val_acc:.4f}, sTest: {s_test_acc:.4f}')
            if (train_acc >= threshold and s_val_acc > best_s_val_acc) or epoch == stage_epochs - 1 or epoch == 0: # epoch % args.eval_every == 0
                ...
                if val_acc > best_val_acc:
                    ...
                    best_val_f1 = val_f1
                    best_test_f1 = test_f1
                    best_preds = val_preds, test_preds
                    best_confidents = val_confidents, test_confidents
                    eval_ids = val_ids, test_ids
                    if args.save_model:
                        torch.save(model.state_dict(), args.save_path + '{}_{}_stage{}.pt'.format(args.logsubfix, run.id, stage_idx))
        ...
        val_ids, test_ids = eval_ids
        val_preds, test_preds = best_preds
        val_confidents, test_confidents = best_confidents
        train_loader = reset_train_dataloader(val_ids, test_ids, val_preds, test_preds, val_confidents, test_confidents, threshold=args.ct)